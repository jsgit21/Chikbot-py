...
--   mysql candyland      < 2026-09-17_reimport_task_data.sql
--   mysql candyland_test < 2026-09-17_reimport_task_data.sql
--
-- On candyland_test only, this must open with a truncate of
-- team_minor_history first: testing has left rows there that FK-reference
-- task.id, which blocks truncating task outright. candyland (production)
-- hasn't run the event yet, so it should have zero rows there; confirmed
-- with `select count(*) from team_minor_history;` before running this on
-- candyland, rather than truncating blind.

-- candyland_test only:
-- truncate table team_minor_history;
